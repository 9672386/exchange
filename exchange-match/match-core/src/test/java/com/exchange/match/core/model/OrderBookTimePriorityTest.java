...
/**
 * 订单薄时间优先测试（订单簿价/量已定点 long,scale 8）。
 */
public class OrderBookTimePriorityTest {
    ...
    /** 十进制字符串 → 定点 long raw(scale 8)。 */
    private static long r(String v) {
        return FixedPoint.fromBigDecimal(new BigDecimal(v), SCALE, RoundingMode.DOWN);
    }
    ...
    public void testTimePriorityForBuyOrders() {
        ...
        assertEquals(r("50000"), buyDepth.get(0).getPrice());
        assertEquals(r("3.0"), buyDepth.get(0).getTotalQuantity());

        // 验证订单在LinkedList中的顺序
        List<Order> orders = orderBook.getBuyOrders().get(r("50000"));
    ...
    public void testTimePriorityForSellOrders() {
        ...
        assertEquals(r("50001"), sellDepth.get(0).getPrice());
        assertEquals(r("3.0"), sellDepth.get(0).getTotalQuantity());

        // 验证订单在LinkedList中的顺序
        List<Order> orders = orderBook.getSellOrders().get(r("50001"));
        assertNotNull(orders);
        assertEquals(3, orders.size());
    ...
    public void testPriceTimePriority() {
        ...
        // 第一个价格层应该是50001
        assertEquals(r("50001"), buyDepth.get(0).getPrice());
        assertEquals(r("1.0"), buyDepth.get(0).getTotalQuantity());

        // 第二个价格层应该是50000
        assertEquals(r("50000"), buyDepth.get(1).getPrice());
        assertEquals(r("2.0"), buyDepth.get(1).getTotalQuantity());

        // 验证同价格内的时间优先
        List<Order> price50000Orders = orderBook.getBuyOrders().get(r("50000"));
    ...
    public void testOrderRemovalMaintainsTimePriority() {
        ...
        // 验证总数量
        List<PriceLevel> buyDepth = orderBook.getBuyDepth(1);
        assertEquals(r("2.0"), buyDepth.get(0).getTotalQuantity());
    }
    ...
    private Order createOrder(String orderId, Long userId, String symbol, OrderSide side,
        ...
        order.setPrice(FixedPoint.fromBigDecimal(price, SCALE, RoundingMode.DOWN));
        order.setQuantity(FixedPoint.fromBigDecimal(quantity, SCALE, RoundingMode.DOWN));
